...
public class MultiBotManager {
	 ...
	 */
	public MultiBotManager(Listener listener) {
	 ...
	 */
	public MultiBotManager(Listener listener, ExecutorService botPool) {
	...
	public void addNetwork(Configuration<PircBotX> config) {
	...
	protected void executeBot(PircBotX bot) throws Exception {
		try {
			...
			ircListener.connectServerError(ex, bot);
			throw ex;
		} catch (Exception ex) {
			ircListener.connectServerError(ex, bot);
			throw ex;
